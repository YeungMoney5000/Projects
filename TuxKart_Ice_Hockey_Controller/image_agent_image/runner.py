...
if __name__ == '__main__':
    ...
    if args.parallel is None or remote.ray is None:
        ...
        #input((train_data[0][0]['team1_images'][0].shape))
        #out = np.empty((6, 300,400))
        #train_images = torch.stack([torch.as_tensor(np.concatenate([d['team1_images'][0], d['team1_images'][1]], axis=2,out=out)) for d in data]).permute(0,3,1,2).to(device).float()/255.
        #train_labels = torch.stack([torch.as_tensor([d['actions'][0]['acceleration'],d['actions'][0]['steer'],d['actions'][0]['brake'],d['actions'][2]['acceleration'],d['actions'][2]['steer'],d['actions'][2]['brake']]) for d in data]).to(device).float()
        ...
        for i in range(args.parallel):
            ...
            train_data = torch.cat([torch.as_tensor(np.array(d['team1_images'])) for d in ray.get(result)])
            train_data = train_data.permute(0, 3, 1, 2).to(device).float() / 255.
     
            labels=[]
            for d in ray.get(result):
                #print(d['team1_state'][0])
                p1= d['team1_state'][0]['camera']['projection'] @ d['team1_state'][0]['camera']['view'].T @ np.array(d['soccer_state']['ball']['location'] + [1])
                p2= d['team1_state'][1]['camera']['projection'] @ d['team1_state'][1]['camera']['view'].T @ np.array(d['soccer_state']['ball']['location'] + [1])
                target1 = np.clip(np.array([p1[0] / p1[-1], -p1[1] / p1[-1]]), -1, 1)
                target2 = np.clip(np.array([p2[0] / p2[-1], -p2[1] / p2[-1]]), -1, 1)
                labels.append([target1,target2])
                #input(labels)
            labels = torch.cat([torch.as_tensor(np.array(points)) for points in labels])
            ...
            with open('image_test.pkl','wb') as f:
    ...
    with open('image_test.pkl','ab') as f:
