...
if __name__ == '__main__':
    ...
    if args.parallel is None or remote.ray is None:
        ...
        for i in range(args.parallel):
            ...
            #input( ray.get(result)[-1]['soccer_state']['score'])
            final_score = ray.get(result)[-1]['soccer_state']['score']
            if final_score [0]>final_score[1]:
                for d in ray.get(result):
                    #print(d['team1_state'][0])
                    data1= extract_featuresV2(d['team1_state'][0], d['soccer_state'], d['team2_state'], 0)
                    data2= extract_featuresV2(d['team1_state'][1], d['soccer_state'], d['team2_state'], 0)
                    label1 =torch.as_tensor([d['actions'][0]['acceleration'],d['actions'][0]['steer'],d['actions'][0]['brake']])
                    label2 =torch.as_tensor([d['actions'][2]['acceleration'],d['actions'][2]['steer'],d['actions'][2]['brake']])
